          ...
          <defs>
            {series.map((s, i) => (
              <linearGradient key={s.key} id={`area-${s.key}`} x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor={s.color ?? chartSeries[i % chartSeries.length]} stopOpacity={0.25} />
                <stop offset="100%" stopColor={s.color ?? chartSeries[i % chartSeries.length]} stopOpacity={0} />
              </linearGradient>
            ))}
          </defs>
          <CartesianGrid strokeDasharray="3 3" stroke={chartColors.grid} vertical={false} />
          <XAxis dataKey={xKey} stroke={chartColors.axis} fontSize={11} tickLine={false} axisLine={false} />
          <YAxis stroke={chartColors.axis} fontSize={11} tickLine={false} axisLine={false} width={32} />
