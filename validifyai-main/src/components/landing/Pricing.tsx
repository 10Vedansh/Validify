    ...
    <section id="pricing" className="py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center mb-14">
          ...
          <h2 className="mt-3 text-3xl sm:text-5xl font-bold tracking-tight">Simple plans. <span className="text-gradient">Serious results.</span></h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {pricing.map((p) => (
            <div key={p.name}
              className={cn("relative rounded-2xl p-7 glass flex flex-col",
                p.featured && "border-primary/50 shadow-glow ring-1 ring-primary/40")}>
              {p.featured && (
              ...
              <Link to="/register" className="mt-7">
                <Button className={cn("w-full", p.featured ? "bg-gradient-primary text-primary-foreground shadow-glow" : "")} variant={p.featured ? "default" : "outline"}>
                  {p.cta}
                </Button>
              </Link>
            </div>
